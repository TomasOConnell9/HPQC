...
void my_MPI_SUM_function(void *input, void *output, int *len, MPI_Datatype *datatype);
...
int root_task(int uni_size, int num_args)
{
	...
	MPI_Op MY_MPI_SUM; //declare variable for new operation
	MPI_Op_create(my_MPI_SUM_function, 1, &MY_MPI_SUM); //create the operation using the function my_MPI_SUM_function

	long long total_sum = 0;
        MPI_Reduce(&root_sum, &total_sum, 1, MPI_LONG_LONG, MY_MPI_SUM, 0, MPI_COMM_WORLD); //implement the bew definded MPI operation
...
void client_task(int num_args, int uni_size, int my_rank)
{
        ...
        MPI_Scatter(NULL, 0, MPI_INT, my_array, chunk, MPI_INT, 0, MPI_COMM_WORLD);
	...
	MPI_Op MY_MPI_SUM; //declare variable
        MPI_Op_create(my_MPI_SUM_function, 1, &MY_MPI_SUM); //create operation

        MPI_Reduce(&sum_of_my_array, NULL, 1, MPI_LONG_LONG, MY_MPI_SUM, 0, MPI_COMM_WORLD); //implement
...
//new function that performs the same operation as MPI_SUM
void my_MPI_SUM_function(void *input, void *output, int *len, MPI_Datatype *datatype)
{
	long long int* in = (long long int*)input;
	long long int* out = (long long int*)output;

	//combine each element in the length of the chunk
	for (int i = 0; i < *len; ++i)
	{
		out[i] += in[i];
	}
