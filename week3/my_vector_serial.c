...
long long int sum_vector(int vector[], int size);
...
int main(int argc, char **argv)
{
	...
	// fill the vector with values from 1 to inputted argument.
	for (int i = 0; i < num_arg; i++)
	{
		my_vector[i] = i + 1;
	}

	// sums the vector, long long to ensure bit size is large enough
	long long int my_sum = sum_vector(my_vector, num_arg);
...
// defines a function to sum a vector of ints into another int, use long long int again to ensure no overflow
long long int sum_vector(int vector[], int size)
{
